...
# CHANGELOG:
# TODO: there is an issue with FREQ - we now get more actions that are the same
...
def read(parser, sentence=None, pos=None, actions=actions, blind_actions=actions, word_freq=word_freq, label_freq=label_freq, strength_of_association={}, decmem={}, lexical=True, visual=True, syntactic=True, reanalysis=True, prints=True):
    ...
    eyemove_times = [] #reaction times, recorded and returned
    ...
    while True:
        ...
        if word_parsed >= len(sentence)-1: #we ignore the last word (period)
            if len(eyemove_times) not in tobe_removed:
    ...
    ACTION_PREV     """+str(parser_retrievals[1]["action"])
            ...
            while True:
                parser_retrievals, _, _, _ = ut.recall_action(actions, parser.goals["imaginal_reanalysis"], parser.goals["word_info"], recently_retrieved, built_constituents_reanalysis, word_freq, label_freq, prints=False, strength_of_association=strength_of_association, number_retrieved=3, postulate_gaps=postulate_gaps, reduce_unary=reduce_unary, blind={})
    ...
    final_times = [x for x in eyemove_times if x not in tobe_removed]
