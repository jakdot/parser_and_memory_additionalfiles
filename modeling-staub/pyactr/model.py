...
class ACTRModel(object):
    ...
    MODEL_PARAMETERS = {"subsymbolic": False,
                ...
                "emma_preparation_time": 0.135 #not used in LispACT-R
                }
    ...
    def __init__(self, environment=None, **model_parameters):
        ...
        try:
            if not set(model_parameters.keys()).issubset(set(self.MODEL_PARAMETERS.keys())):
                raise(utilities.ACTRError("Incorrect model parameter(s) %s. The only possible model parameters are:\n%s" % (set(model_parameters.keys()).difference(set(self.MODEL_PARAMETERS.keys())), "\n".join(sorted(self.MODEL_PARAMETERS.keys())))))
