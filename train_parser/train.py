...
Creates 3 csv files: actions.csv, label.csv and words.csv.
"""
...
#CHANGELOG:
# - 21/02/2020 - added children back, ignoring ACTION_PREV
# - 05/02/2020 - removed all children
        ...
        def _update_action_counter(action, last_action):
            ...
            for pos, x in enumerate(tree_stack[-4:]):
                for elem in x[1]:
                    ...
                    elem = "".join(re.split("=[0-9][0-9]*", "".join(re.split("-[0-9][0-9]*", str(elem)))))
            ...
            if self.future_included == True:
                self.action_counter.update([self.current_action_state(tree0_label=temp_tree_stack[-1][0], tree0_head=lemmatize(temp_tree_stack[-1][1], temp_tree_stack[-1][2], temp_tree_stack[-1][0]), tree0_headpos=temp_tree_stack[-1][2],tree0_leftchild=temp_tree_stack[-1][3], tree0_rightchild=temp_tree_stack[-1][4],\
                    ...
                    tree3_label=temp_tree_stack[-4][0], tree3_head=lemmatize(temp_tree_stack[-4][1], temp_tree_stack[-4][2], temp_tree_stack[-4][0]),\
                    ...
                    next_word0_lex=lemmatize("".join(re.split("-[0-9][0-9]*", next_word0[0])), re.split("\|SPL", next_word0[1])[0]), next_word0_pos=re.split("\|SPL", next_word0[1])[0],\
        ...
        def _reduce(last_action):
            ...
            while True:
                ...
                if tree_stack and (tree_stack[i][0], tree_stack[i][1]) in phrases_in_sent:
                    ...
                    tree_stack[i] = (tree_stack[i][0], (new_phrase,), (tree_stack[i][1][0],), ("",))
                #binary action
